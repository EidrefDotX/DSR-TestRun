...
def create_app() -> Flask:
    ...
    is_production = (
        ...
        bool(os.getenv("PORT"))  # Most platforms set PORT in production
    )
    ...
    def favicon():  # type: ignore[no-untyped-def]
        ...
        svg = """<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 16 16'><rect width='16' height='16' fill='#0e1111'/><text x='8' y='12' text-anchor='middle' font-size='12' fill='white'>X</text></svg>"""
    ...
    def list_users():  # type: ignore[no-untyped-def]
        """List all engineer users (role='client') for admin to manage."""
        user = get_user_from_token()
        if not user or user.get("role") != "admin":
            return jsonify({"ok": False, "error": "Forbidden"}), 403
        db = get_db_session()
        try:
            rows = db.execute(
                select(User).where(User.role == "client").order_by(User.username)
            ).scalars().all()
            items = []
            for r in rows:
                try:
    ...
    @app.put("/admin/users/<string:username>/password")
    def admin_change_user_password(username: str):  # type: ignore[no-untyped-def]
        """Allow admin to change an engineer's password."""
        user = get_user_from_token()
        if not user or user.get("role") != "admin":
            return jsonify({"ok": False, "error": "Forbidden"}), 403
        try:
        ...
        try:
            ...
            # Only allow changing password for engineers (role='client'), not admins
            if target_user.role != "client":
                return jsonify({"ok": False, "error": "Cannot change admin passwords through this endpoint"}), 403
            
            # Update password and history
            new_hash = generate_password_hash(new_password)
            update_password_history(target_user, new_hash)
            target_user.password_hash = new_hash
            db.commit()
            
            print(f"[ADMIN] Password changed for user: {username} by admin: {user.get('username')}")
            return jsonify({"ok": True, "message": f"Password updated for {username}"}), 200
        except Exception as e:
            db.rollback()
            print(f"[ADMIN] Error changing password for {username}: {e}")
...
# Create app instance at module level for Vercel serverless
app = create_app()


if __name__ == "__main__":
    # app is already created at module level above
    ...
    print("\n" + "="*60)
    print("SERVER RUNNING (Production WSGI - Waitress)")
    ...
    try:
        ...
        if "Address already in use" in str(e) or "address is already in use" in str(e).lower():
            ...
            print(f"Kill the process: netstat -ano | findstr :{port}")
        else:
